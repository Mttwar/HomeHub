...
function SpendingChart({ total, values }: { total: number; values: number[] }) {
  const visible = values.slice(-6);
  const max = Math.max(...visible, 1);
  const points = visible.map((value, index) => `${index * 99},${130 - (value / max) * 110}`).join(" ");
  return (
    <article className="rounded-[28px] border border-white bg-white p-5 shadow-[0_12px_40px_rgba(15,23,42,.055)] sm:p-6">
      ...
      <div className="mt-6 flex items-end gap-3"><span className="text-3xl font-bold tracking-[-0.05em]">{new Intl.NumberFormat("it-IT", { style: "currency", currency: "EUR" }).format(total)}</span><span className="mb-1 flex items-center gap-1 text-xs font-bold text-emerald-600"><TrendingDown className="size-3.5" /> dati registrati</span></div>
      <div className="mt-6 h-40"><svg viewBox="0 0 500 140" preserveAspectRatio="none" className="h-full w-full" role="img" aria-label="Grafico dimostrativo delle spese mensili"><defs><linearGradient id="spendFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#7657FF" stopOpacity="0.24" /><stop offset="1" stopColor="#7657FF" stopOpacity="0" /></linearGradient></defs>{[20, 60, 100, 140].map((y) => <line key={y} x1="0" y1={y} x2="500" y2={y} stroke="#E8EAF0" strokeDasharray="4 6" />)}<path d={`M ${points} L 495 140 L 0 140 Z`} fill="url(#spendFill)" /><polyline points={points} fill="none" stroke="#7657FF" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" /><circle cx="385" cy="27" r="5" fill="#fff" stroke="#7657FF" strokeWidth="3" /></svg></div>
      <div className="mt-2 grid grid-cols-6 text-center text-[10px] font-semibold text-slate-400">{["FEB", "MAR", "APR", "MAG", "GIU", "LUG"].map((month) => <span key={month}>{month}</span>)}</div>
    </article>
  );
}
