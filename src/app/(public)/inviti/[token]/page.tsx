  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f5f6fa] p-5">
      <section className="w-full max-w-lg rounded-[32px] bg-white p-6 text-center shadow-[0_18px_60px_rgba(15,23,42,.08)] sm:p-9">
        ...
        {available && session?.user && !correctAccount && <div className="mt-6 rounded-2xl bg-amber-50 p-4 text-left text-xs leading-5 text-amber-800"><CheckCircle2 className="mb-2 size-4" />Hai effettuato l’accesso come <b>{session.user.email}</b>. Esci e accedi con l’indirizzo destinatario dell’invito.<br /><SignOutButton callbackURL={callbackURL} /></div>}
