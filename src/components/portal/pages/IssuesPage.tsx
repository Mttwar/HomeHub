...
export function IssuesPage({ query, role, records = [], onOpenModal }: { query: string; role: Role; records?: IssueListItem[] | undefined; onOpenModal: (kind: ModalKind) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-4">{columns.map((column, index) => { const cards = filtered.filter((issue) => issue.status === column); return <section key={column} className="issue-column rounded-[24px] bg-slate-100/80 p-3"><div className="flex items-center justify-between px-2 py-2"><div className="flex items-center gap-2"><span className={cx("size-2 rounded-full", columnTones[index])} /><h2 className="text-xs font-extrabold uppercase tracking-[.11em] text-slate-600">{column}</h2></div><span className="text-xs font-bold text-slate-400">{cards.length}</span></div><div className="mt-2 space-y-3">{cards.map((issue) => <article key={issue.id} className="issue-card rounded-[20px] border border-white bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,.05)]"><span className="rounded-full bg-slate-100 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-slate-500">{issue.category}</span><h3 className="mt-4 text-sm font-bold leading-5">{issue.title}</h3><p className="mt-2 line-clamp-3 text-xs leading-5 text-slate-500">{issue.description}</p><div className="mt-4 flex items-center justify-between text-[10px] text-slate-400"><span>{issue.date}</span><span className="flex items-center gap-1"><MessageCircle className="size-3" />{issue.comments}</span></div>{issue.priority === "Urgente" && <div className="mt-3 rounded-xl bg-rose-50 px-2.5 py-2 text-[10px] font-bold text-rose-600">Intervento urgente</div>}
        ...
        </article>)}{cards.length === 0 && <div className="issue-empty rounded-[20px] border border-dashed border-slate-200 bg-white/50 p-6 text-center text-xs text-slate-400">Nessuna segnalazione</div>}</div></section>; })}</div>
    </div>
  );
}
